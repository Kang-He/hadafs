...
__iobuf_arena_alloc (struct iobuf_pool *iobuf_pool)
{
		...
		gf_log("iobuf", GF_LOG_ERROR, "arana [arena_size=%ld] mmap failed %s",
	...
	if (iobuf_arena->mem_base == NULL){
		gf_log("iobuf", GF_LOG_ERROR, "arana [arena_size=%ld] valloc failed %s",
			arena_size, strerror(errno));
                goto err;
	}
#endif
...
iobuf_get (struct iobuf_pool *iobuf_pool)
{
        ...
        pthread_mutex_lock (&iobuf_pool->mutex);
        {
                ...
                if (!iobuf_arena){
#ifndef XIAOW20180925
			gf_log("iobuf", GF_LOG_ERROR, "iobuf_arena empty");
#endif
                        goto unlock;
		}
                ...
                if (!iobuf){
#ifndef XIAOW20180925
			gf_log("iobuf", GF_LOG_ERROR, "iobuf empty");
#endif
                        goto unlock;
		}
                __iobuf_ref (iobuf);
