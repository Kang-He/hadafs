...
{
    if (gf_log_loglevel = GF_LOG_NONE)
        return 0;
...
void 
gf_log_dump_backtrace(char *key)
{
	void *array[200];
	size_t size;
	...
	gf_log("back_trace", GF_LOG_NORMAL, "------------start %s------------", key);
#if HAVE_BACKTRACE
	size = backtrace (array, 200);
	backtrace_symbols_fd (&array[1], size-1, fileno(gf_log_logfile));
#endif
	gf_log("back_trace", GF_LOG_NORMAL, "------------end %s------------", key);
}
