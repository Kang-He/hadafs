...
const char *argp_program_version = "" \
            ...
            "Copyright (c) 2006-2009 HADA Inc. "             \
            "<http://www.hada.com>\n"                                \
...
gf_daemon (int *pipe_fd)
{
        ...
        if (pipe (pipe_fd) < 0) {
                gf_log ("hadafs", GF_LOG_ERROR,
                        "pipe creation error- %s", strerror (errno));
                return -1;
        }

        if ((pid = fork ()) < 0) {
	        gf_log ("hadafs", GF_LOG_ERROR, "fork error: %s",
			strerror (errno));
                return -1;
        } else if (pid != 0) {
                close (pipe_fd[1]);
...
static xlator_t *
_parse_specfp (hadafs_ctx_t *ctx, 
	       FILE *specfp)
{
        int spec_fd = 0;
	cmd_args_t *cmd_args = NULL;
...
cleanup_and_exit (int signum)
{
	...
	gf_log ("hadafs", GF_LOG_WARNING, "shutting down");
	...
	if (ctx->graph) {
		...
		gf_log ("hadafs", GF_LOG_DEBUG, "no graph present");
	}
}
...
main (int argc, char *argv[])
{
	...
	if ((specfp = _get_specfp (ctx)) == NULL) {
		...
		gf_log ("hadafs", GF_LOG_ERROR, "exiting\n");
	...
	gf_log ("hadafs", GF_LOG_NORMAL, "Successfully started");
